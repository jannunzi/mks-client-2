...
const unhighlightTable = () => {
  $(".rjsf #root > .form-group.field.field-array .row.array-item-list .array-item")
    .css('backgroundColor', 'transparent')
  $("[id='root_Hopping Presets__title']").parent().parent()
    .css('backgroundColor', 'transparent')
}
...
export default (parentComponent) =>
  $("body")
    .on("copy", "[id='root_Hopping Presets__title']", (event) => {

      // COPY ALL BINS AND PRESETS FROM TABLE TO CLIPBOARD

      parentComponent.setState(prevState => {

        // grab current presets from current state
        const data = prevState.formData['Hopping Presets']
    ...
    .on('copy', '.rjsf #root > .form-group.field.field-array .row.array-item-list > .array-item legend', (event) => {

      // COPY SELECTED PRESET COLUMN TO CLIPBOARD
      ...
      for(let i=0; i<inputs.length; i++) {
        ...
        copiedValues.push(parseInt($(inputs[i]).val()) || null)
      }
    ...
    .on("copy", "[id='root_Hopping Presets'] .array-item .array-item", (event) => {

      // COPY SELECTED BIN ROW TO CLIPBOARD
    ...
    .on("paste", "[id='root_Hopping Presets'] .array-item .array-item", (event) => {
      event.preventDefault();
    ...
    .on("paste", "[id='root_Hopping Presets__title']", (event) => {
    ...
    .on('paste', '.rjsf #root > .form-group.field.field-array .row.array-item-list > .array-item legend', (event) => {
      ...
      parentComponent.setState(prevState => {
        ...
        return ({
          formData: newFormData,
          past: [...prevState.past, newFormData ] //prevState.formData ]
        })
      })
    })
    .on('click', '.rjsf #root > .form-group.field.field-array .row.array-item-list > .array-item legend', (event) => {
    ...
    .on("click", "[id='root_Hopping Presets__title']", (event) => {

      // HIGHLIGHT TABLE
      unhighlightTable()

      $(event.target).parent().parent().css('backgroundColor', HIGHLIGHT_COLOR)
    ...
    .on("click", "[id='root_Hopping Presets'] .array-item .array-item label", (event) => {
